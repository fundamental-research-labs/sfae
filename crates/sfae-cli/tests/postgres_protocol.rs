...
const CREDENTIAL_ID: &str = "00000000-0000-4000-8000-000000000077";
...
impl PostgresContainer {
    ...
    fn refresh_endpoint(&mut self) {
        let endpoint = postgres_endpoint(&self.name);
...
fn docker(args: DockerArgs<'_>) -> std::process::Output {
...
fn assert_success(ctx: CommandOutputCtx) {
...
fn unique_name() -> String {
    let nanos = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    format!("sfae-postgres-protocol-{}-{nanos}", std::process::id())
}
...
fn postgres_endpoint(name: &str) -> PostgresEndpoint {
    if let Some(port) = mapped_postgres_port(name) {
        let mapped = PostgresEndpoint {
            host: "127.0.0.1".to_string(),
            port,
        };
...
fn mapped_postgres_port(name: &str) -> Option<String> {
    let output = docker(DockerArgs {
...
fn container_ip(name: &str) -> String {
    ...
    assert_success(CommandOutputCtx {
        action: "inspect postgres container IP",
        output,
    });
    if stdout.is_empty() {
        panic!("postgres container has no bridge IP");
    }
    stdout
}

fn tcp_reachable(endpoint: &PostgresEndpoint) -> bool {
...
fn command_with_store(store: &MockCredentialStore) -> Command {
