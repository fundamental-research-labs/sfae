...
- [x] 3b: Merge `done.html` and `oauth_done.html` into a single `done.html` template with `{{TITLE}}` and `{{HEADING}}` placeholders. Update `build_done_page()` and `build_oauth_done_page()` in `browser.rs` to pass appropriate values ("sfae — done" / "Credential saved" vs "sfae — authorized" / "Authorized"). Delete `oauth_done.html`.
