...
- [ ] 1a: Add `PromptSpec`, `FieldSpec`, `GroupSpec`, `OAuthSpec` serde structs to sfae-core (new module `spec.rs`). Support string shorthand for fields (custom deserializer: `"API_KEY"` → `FieldSpec { name: "API_KEY", .. }`). Validate: at least one of `fields`/`groups` present; groups have either `fields` or `oauth` not both. Add `FieldSpec::is_secret()` auto-detection and `FieldSpec::display_label()` (humanizes name, e.g. `ACCESS_TOKEN` → `"Access Token"`).
